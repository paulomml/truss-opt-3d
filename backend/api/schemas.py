...
class RequisicaoOtimizacao(BaseModel):
    ...
    width: float = Field(..., ge=0, description="Largura da seção transversal (W) em metros. 0 = análise 2D.")
    ...
    raw_truss: Optional[TrelicaBruta] = None

    # Parâmetros de vento NBR 6123.
    parametros_vento: Optional[ParametrosVento] = None

    # Restrições do espaço de busca do GA.
    restricoes: Optional[RestricoesOtimizacao] = None

    # Parâmetros do GA (sobrepõe configurações padrão).
    ag_geracoes: Optional[int] = Field(None, ge=1, le=200)
...
class StatusTarefa(BaseModel):
    ...
    resultado: Optional[RespostaOtimizacao] = None
...
class PerfilSchema(BaseModel):
    ...
    chapa_referencia: Optional[str] = None
