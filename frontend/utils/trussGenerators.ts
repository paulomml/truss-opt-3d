...
function createNodeId(prefix: string, index: number, suffix: string = ''): string {
...
function extrude3D(planar: RawTruss, width: number): RawTruss {
  ...
  for (const m of planar.members) {
    ...
    if (Math.abs(n1.x - n2.x) > 0.01) {
      addMember(members, m.node_start + 'F', m.node_end + 'B', 'Contraventamento');
      addMember(members, m.node_start + 'B', m.node_end + 'F', 'Contraventamento');
    }
  }
...
export function generatePrattRoof(
  ...
  for (let i = 0; i <= panels; i++) {
    ...
    addNode(nodes, bId, x, 0, 0, i === 0 ? 'Pinned' : i === panels ? 'Roller' : 'None');

    const ty =
      i <= panels / 2 ? (i * dx * height) / (span / 2) : ((panels - i) * dx * height) / (span / 2);
    if (i > 0 && i < panels) {
  ...
  if (panels % 2 === 0)
    addMember(members, createNodeId('B', panels / 2), createNodeId('T', panels / 2), 'Montante');
...
export function generateFinkRoof(span: number, height: number, width: number): RawTruss {
  // Treliça Fink.
  span = Math.max(0.1, span);
  height = Math.max(0.1, height);
  width = Math.max(0, width);
  ...
  for (let i = 0; i < panels; i++) {
    addMember(members, createNodeId('B', i), createNodeId('B', i + 1), 'Banzo Inferior');
    const tC = i === 0 ? createNodeId('B', 0) : createNodeId('T', i);
    const tN = i + 1 === panels ? createNodeId('B', panels) : createNodeId('T', i + 1);
    addMember(members, tC, tN, 'Banzo Superior');
  }
...
export function generateWarrenBridge(
  ...
  for (let i = 0; i < panels; i++) addNode(nodes, createNodeId('T', i), (i + 0.5) * dx, height, 0);

  for (let i = 0; i < panels; i++) {
    addMember(members, createNodeId('B', i), createNodeId('B', i + 1), 'Banzo Inferior');
    if (i < panels - 1)
      addMember(members, createNodeId('T', i), createNodeId('T', i + 1), 'Banzo Superior');
    addMember(members, createNodeId('B', i), createNodeId('T', i), 'Diagonal');
    addMember(members, createNodeId('T', i), createNodeId('B', i + 1), 'Diagonal');
  }
...
export function generatePrattBridge(
  ...
  for (let i = 0; i < panels; i++) {
    ...
    if (i === panels - 1)
      addMember(members, createNodeId('B', i + 1), createNodeId('T', i + 1), 'Montante');
    if (i < panels / 2)
      addMember(members, createNodeId('B', i), createNodeId('T', i + 1), 'Diagonal');
    else addMember(members, createNodeId('T', i), createNodeId('B', i + 1), 'Diagonal');
  }
...
export function generateSquareTower(
  ...
  for (let i = 0; i <= sections; i++) {
    ...
    if (width === 0) {
      ...
      if (i > 0) {
        addMember(members, createNodeId('N1', i - 1), createNodeId('N1', i), 'Principal');
        addMember(members, createNodeId('N2', i - 1), createNodeId('N2', i), 'Principal');
        addMember(members, createNodeId('N1', i - 1), createNodeId('N2', i), 'Diagonal');
        addMember(members, createNodeId('N2', i - 1), createNodeId('N1', i), 'Diagonal');
      }
      addMember(members, createNodeId('N1', i), createNodeId('N2', i), 'Transversal');
    } else {
      // Geração espacial 3D.
      ...
      if (i > 0) {
        for (let j = 1; j <= 4; j++) {
          ...
          addMember(members, createNodeId('N' + j, i - 1), createNodeId('N' + j, i), 'Montante');
          addMember(
            members,
      ...
      addMember(members, createNodeId('N1', i), createNodeId('N2', i), 'Transversal');
      addMember(members, createNodeId('N2', i), createNodeId('N3', i), 'Transversal');
      addMember(members, createNodeId('N3', i), createNodeId('N4', i), 'Transversal');
      addMember(members, createNodeId('N4', i), createNodeId('N1', i), 'Transversal');
    }
  }
  return { nodes, members };
}
...
export function generateTriangularTower(
  ...
  for (let i = 0; i <= sections; i++) {
    ...
    if (width === 0) {
      ...
      if (i > 0) {
        addMember(members, createNodeId('N1', i - 1), createNodeId('N1', i), 'Montante');
        addMember(members, createNodeId('N2', i - 1), createNodeId('N2', i), 'Montante');
        addMember(members, createNodeId('N1', i - 1), createNodeId('N2', i), 'Diagonal');
      }
      addMember(members, createNodeId('N1', i), createNodeId('N2', i), 'Transversal');
    } else {
      ...
      addMember(members, createNodeId('N1', i), createNodeId('N2', i), 'Transversal');
      addMember(members, createNodeId('N2', i), createNodeId('N3', i), 'Transversal');
      addMember(members, createNodeId('N3', i), createNodeId('N1', i), 'Transversal');
    }
  }
  return { nodes, members };
}
...
export function generateCantileverPratt(
  ...
  for (let i = 0; i <= panels; i++) {
    ...
    addNode(nodes, createNodeId('T', i), x, height, 0, i === 0 ? 'Fixed' : 'None');
  }
...
export function generateCantileverWarren(
  ...
  for (let i = 0; i <= panels; i++)
    addNode(nodes, createNodeId('B', i), i * dx, 0, 0, i === 0 ? 'Fixed' : 'None');
  for (let i = 0; i < panels; i++) addNode(nodes, createNodeId('T', i), (i + 0.5) * dx, height, 0);
